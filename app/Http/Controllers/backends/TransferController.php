...
class TransferController extends Controller {
    public function index(Request  $request){
        ...
        if($user->can('transfer.read')){
            ...
            $eq_query = function ($query) use ($keyword){
                return $query->select('id','title')->where('equipments.title','like','%'.$keyword.'%');
            };
            if($keyword != '') $transfers = $transfers->whereHas('transfer_equipment', $eq_query);
            if($status_key != '') $transfers = $transfers->where('status',$status_key);
    ...
    public function create(){
        ...
        if($user->can('create', Transfer::class)){ 
            $equipments = Equipment::select('id','title')->where('department_id',$user->department_id)->where('status','!=','was_broken')->get();
    ...
    public function store(Request  $request){
        ...
        $messages = [
            ...
            'amount.required'=>'Vui lòng nhập số lượng không ít hơn 0 !',
        ...
        if(intval($equipment->amount) >= intval($request->amount)){
            ...
            return redirect()->back()->with('error','Thêm thành không công nhập quá số lượng');
        }
        endif;
    }
    ...
    public function update(Request  $request , $id){
        ...
        if($request->status == "public"){
            ...
            if($transfers->wasChanged('status'))
    ...
    public function getQuantity(Request  $request){
        $equipment = Equipment::select('id','title','amount')->where('id',$request->id)->first();
